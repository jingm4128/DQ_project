...
select current_date(), current_date()-90, current_date()- INTERVAL 30 DAY;
		...
		select 'auto_r' as model, ticker, avg(adj_close) as price_mode, 2*stddev(adj_close) as price_band, -0.5 as price_skew, 4.4 as alpha, 8.8 as beta
        from us.equity_daily_recent_alphav
        where date>='2018-06-01'
        group by 1,2,5,6,7;   
...
# asof_datetime, model, ticker, price_mode, price_band, price_shape, price_skew, alpha, beta, deprecated
